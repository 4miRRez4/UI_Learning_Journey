//https://ramtung.ir/apnotes/html/10_Exceptions.html
//-just printing   -return value   -using global variable   -using exceptions(throw and catch approach)
#include <iostream>
#include <string>
#include <vector>
#include <fstream>
...
void read_student_info(char* filename, vector<Student>& v) {
    ...
    for (int i = 0; i < count; i++) {
        try {
            ...
            cerr << "date Error in line #" << (i+1) << endl;
        } catch (Bad_Name_Exception nex)
        {
            cerr << "name Error in line #" << (i+1) << endl;
        }
    }
}
