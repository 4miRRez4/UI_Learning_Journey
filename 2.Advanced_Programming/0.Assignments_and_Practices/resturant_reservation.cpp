...
public:
	...
	void add_new_employee(string id, string name) {
...
int main() {
	Ehtesham ehtesham(
		{
			new Catering("Farsi", { MenuItem("Kabab", 50000), MenuItem("Jooj", 30000) }),
			new Catering("Alaa", { MenuItem("Kabab", 40000), MenuItem("Maahi", 35000) })
		},
		{
			new Employee("1001", "Jamshid"),
			new Employee("1002", "Mahvash"),
			new Employee("1003", "Hooshang")
		}
	);
	...
	while (cin >> command) {
		...
		} else if (command == "newemployee") {
			string empid;
			string empname;
			cin >> empid >> empname;
			ehtesham.add_new_employee(empid, empname);
